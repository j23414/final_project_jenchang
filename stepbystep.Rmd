...
data<-subset(data,select=c(Name:Probes,C.2:PH1,SD1:UV2)) # Drop C1 and PH2, (bubble on the array)
ggparcoord(data, columns=c(7:24))+geom_line()+labs(title="Expression Across All Treatments", x="Treatments", y="Expression")+theme(axis.text.x=element_text(angle=270))
```
...
#ggparcoord(data, columns=c(7:24),groupColumn=25)+geom_line()+labs(title="Clustered Genes", x="Treatments", y="Expression")+theme(axis.text.x=element_text(angle=270))

# facet by cluster
#library(dplyr)
#library(plyr)
library(reshape)
data.m<-melt.data.frame(data[,c(1,7:25)], id.vars=c("Name","cluster"))
...
plot(g1)
plot(g1,"neato")
rEG <- new("graphNEL", nodes=c("A", "B"), edgemode="undirected")
